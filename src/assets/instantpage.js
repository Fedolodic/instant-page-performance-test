...
function init() {
  ...
  const isSupported =
    supportCheckRelList.supports('prefetch') && supportCheckRelList.supports('modulepreload');
  ...
  if ('instantIntensity' in document.body.dataset) {
    ...
    if (intensityParameter == 'viewport') {
      ...
      const isNavigatorConnectionAdequate =
        !isNavigatorConnectionSaveDataEnabled && !isNavigatorConnectionLike2g;
...
function isEventLikelyTriggeredByTouch(event) {
  ...
  return (
    durationBetweenLastTouchstartAndNow < MAX_DURATION_TO_BE_CONSIDERED_TRIGGERED_BY_TOUCHSTART;
  );
...
function isPreloadable(anchorElement) {
  ...
  if (
    anchorElement.hash &&
    anchorElement.pathname + anchorElement.search == location.pathname + location.search
  ) {
    return;
  }
